...
void solve() {
    ...
    rep(u, q) {
        ...
        debug(l)
        rep(i, 26) {
            left[i]+=pref[i][m]-(l?pref[i][l-1]:0);
        }
        rep(i, 26) {
            right[i]+=pref[i][r]-(m?pref[i][m-1]:0);
        }
        debug(left) debug(right)
        int ok = 0, flg=0;
        if(pref[s[m]-'a'][m]-(m?pref[s[m]-'a'][m-1]:0)) right[s[m]-'a']--,flg=1;
        rep(i, 26) {
            vi nleft=left,nright=right;
            if(pref[i][m]-(l?pref[i][l-1]:0)) {
                nleft[i]--;
                if(nleft==nright) {
                    ok=1;break;
                }
        ...
        if(flg) right[s[m]-'a']++, left[s[m]-'a']--;
        rep(i, 26) {
            vi nleft=left,nright=right;
            if(pref[i][r]-(m?pref[i][m-1]:0)) {
                nright[i]--;
                if(nleft==nright) {
                    ok=1;break;
                }
