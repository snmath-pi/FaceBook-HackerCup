...
void solve() {
    ...
    rep(i, n) cin >> a[i];
    rep(i, n) cin >> b[i];
    // check if b is ever contained in a
    // for that we can use kmp algorithm for string matching
    // to prevent looping just create a copy of array 'a' two times
    vi c=a;
    rep(i, n) c.pb(a[i]);
    debug(c)
    vi beta(2*n);
    auto pre=[&]() {
        int i=0, j=-1; beta[0]=-1;
        while(i<n) {
            while(j>=0&&beta[i]!=beta[j]) j = beta[j];
            i++,j++;
            beta[i]=j;
        }
    };
