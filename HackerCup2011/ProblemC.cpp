...
string dp[1<<10];
string dfs(int mask, vector<string>& a) {
    if(mask==(1<<n)-1) return "";
    string &ret = dp[mask];
    if(ret!="{") return ret;
    rep(i, n) {
        if(!(mask>>i & 1)) {
            chkmin(ret, a[i]+dfs(mask|(1<<i), a));
        }
    }
    return ret;
}
...
void solve() {
    cin >> n; vector<string> a(n); rep(i, n) cin >> a[i];
    sort(all(a), [&](string x, string y) {return x+y<y+x;});
    string res;
    rep(i, n) res+=a[i]; cout << res << nl;
}
int main() {
#ifndef ONLINE_JUDGE
    freopen("error.txt", "w", stderr);
