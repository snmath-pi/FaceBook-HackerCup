...
void solve() {
    ...
    map<char,int> mp; 
    for(auto &x: s) mp[x]++;
    int mx = max({mp['A'], mp['E'], mp['I'], mp['O'], mp['U']});
    ll tot = mp['A']+ mp['E']+ mp['I']+ mp['O']+ mp['U']-mx;
    int mx2 = 0, uler=0;
    for(auto &x: mp) {
        if(x.fi!='A'&&x.fi!='E'&&x.fi!='I'&&x.fi!='O'&&x.fi!='U') {uler+=x.se, chkmax(mx2,x.se);}
    }
    cout << min(2*tot+uler, tot+mx+2*(uler-mx2)) << nl;
